...
export async function insertShortLinkPair(slPair: shortLinkDto) {
...
export async function getShortLinkPair(slug: string): Promise<shortLinkDto> {
    const shortLinks = await db
        .select()
        .from(shortLinkTable)
        .where(eq(shortLinkTable.slug, slug));

    let returnValue: shortLinkDto = {
        slug: slug
